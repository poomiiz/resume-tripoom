      ...
      <div className="space-y-12">
        {CAREER_ARC_STEPS.map((step, idx) => (
          <div key={step.id} className="relative flex flex-col md:flex-row items-start md:items-center group">
            ...
            {/* Content Card */}
            <article className={`portfolio-panel rounded-2xl p-5 md:p-6 w-full md:w-[calc(50%-2rem)] transition-all duration-300 group-hover:bg-white/5 border border-white/5 ${
              ...
              <LocaleStack
                ...
                locale={locale}
                as="p"
                className="text-[0.65rem] font-bold uppercase tracking-[0.2em] text-[color:var(--pf-accent)]"
              />
              <LocaleStack text={step.title} locale={locale} as="h3" className="mt-1 text-base md:text-lg font-bold" />
              <LocaleStack text={step.body} locale={locale} as="p" className="mt-2 text-sm leading-relaxed opacity-70" />
            </article>

            {/* Year marker for desktop side */}
            <div className={`hidden md:block absolute top-1/2 -translate-y-1/2 text-[0.65rem] font-bold uppercase tracking-widest opacity-20 w-[calc(50%-3rem)] ${
